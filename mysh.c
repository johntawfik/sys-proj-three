...
void process_wildcard(char* pattern, int interactive){
    ...
    while(*second_part != '\0' && isspace(*second_part)) second_part++;
    glob_t glob_result;
    ...
    if (return_value == 0) {
        for (size_t i = 0; i < glob_result.gl_pathc; ++i) {
            ...
            if (full_command) {
                ...
                execute_command(full_command, interactive);
                free(full_command);
            }
        }
    } else if (return_value == GLOB_NOMATCH) {
        execute_command(pattern, interactive);
    }
...
char* emulate_which(char* file_name){
    if(contains_more_than_one_space(file_name)) return NULL;
     char full_path[1024]; 
...
void processInput(int fd, int interactive) {
    ...
    while ((bytesRead = read(fd, buffer, BUFFER_SIZE - 1)) > 0) {
        ...
        if(strchr(cmd, '*') != NULL) process_wildcard(cmd), interactive;
        while (cmd != NULL) {
